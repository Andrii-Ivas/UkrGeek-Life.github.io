...
LINKS = {
    ...
    "SPOTIFY_EMBED": "https://open.spotify.com/embed/playlist/1BG0k1dDJN14PczoyvSD6T?utm_source=generator&theme=0"
}
VIDEO_ID = "-h7ygd0mp7c"

def run(cmd):
    try: subprocess.run(cmd, shell=True, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL); print(f"✅ {cmd}")
    except: pass
...
<div class="code-block">
    ...
    <a href="{LINKS['YOUTUBE']}" target="_blank" class="code-item">  YT: <span class="str">"Sub"</span>,</a>
    <a href="{LINKS['INSTA_MAIN']}" target="_blank" class="code-item">  Insta_Life: <span class="str">"Follow"</span>,</a>
    <a href="{LINKS['INSTA_PHOTO']}" target="_blank" class="code-item">  Insta_Art: <span class="str">"View"</span>,</a>
...
<div style="margin-top:50px;border-top:1px dashed #0F0;padding-top:20px;">
    ...
    <iframe style="border-radius:12px;margin-top:10px;" src="{LINKS['SPOTIFY_EMBED']}" width="100%" height="152" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>
</div>
"""
ABOUT_CONTENT = BASE_ABOUT + CODE_LINKS

# PHOTO GRID (4 COLS FIXED)
WIDGET = """<div class="widget-box"><div class="widget-header">CAM_FEED: LIVE</div><div class="widget-content"><iframe src="https://snapwidget.com/embed/1115084" class="snapwidget-widget" allowtransparency="true" frameborder="0" scrolling="no" style="border:none; overflow:hidden; width:100%; height:100%" title="Posts from Instagram"></iframe></div></div>"""
PHOTO_CONTENT = f"""<h1>/GALLERY_GRID</h1><p>Visual Database Active.</p><div class="photo-grid-container">{WIDGET*8}</div>"""

# TERMINAL
CONTACT_CONTENT = """<h1>Terminal</h1><div class='terminal-window'><div id='history'><p>UkrGeekLife OS v28.0 (Alien Edition)...</p><p>Type 'help'.</p></div><div class='input-line'><span class='prompt'>root@ukrgeek:~#</span><input type='text' id='cmd' autofocus autocomplete='off' spellcheck='false'></div></div>"""
...
#preloader{position:fixed;top:0;left:0;width:100%;height:100%;background:#000;z-index:9999;display:flex;align-items:center;justify-content:center;flex-direction:column;color:#0F0;font-family:monospace;transition:opacity 1s}
...
.id-project{cursor:pointer;padding:5px;border:1px solid transparent;transition:0.3s} .id-project:hover{color:#F00;border-color:#F00;text-shadow:0 0 10px #F00}
.id-name{text-decoration:none;color:#FFF;padding:5px;transition:0.3s} .id-name:hover{color:#0F0;text-shadow:0 0 10px #0F0}
.hal-mini-eye{width:15px;height:15px;background:#500;border-radius:50%;box-shadow:0 0 5px #F00;display:inline-block;margin-right:5px}
.id-project:hover .hal-mini-eye{background:#F00;box-shadow:0 0 15px #F00}

/* HEADER MODULES GROUP */
.header-modules { display: flex; align-items: center; gap: 20px; }

/* 1. HAL 9000 EGG */
.hal-9000-unit {
    width: 30px; height: 30px; background: #200; border-radius: 50%;
    border: 2px solid #555; display: flex; align-items: center; justify-content: center;
    cursor: pointer; transition: 0.3s;
}
.hal-lens { width: 10px; height: 10px; background: #F00; border-radius: 50%; box-shadow: 0 0 5px #F00; }
.hal-9000-unit:hover { border-color: #FFF; }
.hal-9000-unit:hover .hal-lens { box-shadow: 0 0 15px #F00; transform: scale(1.2); }

/* 2. ALIEN RADAR (VISUAL ONLY) */
.radar-display {
    width: 40px; height: 40px; background: #001; border-radius: 50%;
    border: 2px solid #050; position: relative; overflow: hidden;
...
.radar-dot {
    position: absolute; width: 4px; height: 4px; background: #0F0; border-radius: 50%;
    top: 50%; left: 50%; transform: translate(-50%, -50%);
    box-shadow: 0 0 5px #FFF; transition: all 0.1s;
}
@keyframes radar-spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }

/* 3. DONATE BUTTON (CYBER STYLE) */
.donate-btn {
    text-decoration: none; color: #000; background: #0F0;
    font-weight: bold; padding: 5px 15px; font-family: monospace;
    border: 1px solid #0F0; box-shadow: 0 0 10px #0F0;
    text-transform: uppercase; letter-spacing: 1px;
    transition: 0.3s;
}
.donate-btn:hover { background: #000; color: #0F0; box-shadow: 0 0 20px #0F0; }

.header-social{display:flex; gap: 10px;}
.social-icon{color:#555;font-size:1.1rem;text-decoration:none;transition:0.3s} .social-icon:hover{color:#FFF;text-shadow:0 0 5px #FFF}

#monolith-menu{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.95);z-index:2000;display:none;flex-direction:column;align-items:center;justify-content:center;backdrop-filter:blur(5px)}
#monolith-menu.active{display:flex}
.hal-big-eye{width:80px;height:80px;background:#300;border-radius:50%;border:4px solid #888;box-shadow:0 0 20px #F00;margin-bottom:40px;cursor:pointer;transition:0.3s}
.hal-big-eye:hover{background:#F00;box-shadow:0 0 50px #F00;transform:scale(1.1)}
...
.code-block{background:#0a0a0a;border-left:3px solid #0F0;padding:15px;font-family:'Consolas',monospace;margin-top:20px}
.kwd{color:#d7ba7d} .var{color:#9cdcfe} .str{color:#ce9178}
...
.widget-content{flex:1;overflow:hidden} .widget-content iframe{width:100%;height:100%;border:none}
.terminal-window{background:#111;border:1px solid #0F0;padding:15px;height:50vh;overflow-y:auto;text-align:left}
...
.footer-links a{color:#555;margin:0 10px;font-size:1.2rem;transition:0.3s} .footer-links a:hover{color:#FFF}
"""
...
<header>
    <div class="identity-wrapper"><div class="id-project" onclick="toggleMenu()">[{ID_PROJECT}]<div class="hal-mini-eye"></div></div><span class="id-divider">::</span><a href="index.html" class="id-name">{ID_NAME}</a></div>
    ...
    <div class="header-modules">
        ...
        <a href="{DONATE_LINK}" target="_blank" class="donate-btn">DONATE</a>
...
time.sleep(1)
run("git add .")
run(f'git commit -m "UkrGeekLife | HEADER MODULES & RADAR | {time.strftime("%H:%M:%S")}"')
